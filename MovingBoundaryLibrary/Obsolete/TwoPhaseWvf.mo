...
model TwoPhaseWvf
  "1-D lumped fluid flow model for two-phase flow - Constant Void Fraction"
  import Components;
...
/************ Thermal and fluid ports ***********/
 ...
 ThermoCycle.Interfaces.Fluid.FlangeB outFlow(redeclare final package Medium =
  ...
  Units.HeatExchangers.MovingBoundary.Interfaces.MbOut mbOut annotation (Placement(transformation(extent={{-10,80},{10,100}})));
...
/* Derivative of fluid properties */
 ...
 parameter Real VV = 0.8 "Average void fraction";
...
dMdt = AA*(der(ll)*(VV*rho_v + (1-VV)*rho_l) + ll*(VV*drdp_v*der(pp) + (1-VV)*drdp_l*der(pp)))
+ rho_a*AA*der(la) - rho_b*AA*der(lb);
...
dUdt = AA*(der(ll)*(VV*rho_v*h_v + (1-VV)*rho_l*h_l) + ll*(VV*drdp_v*der(pp)*h_v
+VV*rho_v*dhdp_v*der(pp) +(1-VV)*drdp_l*der(pp)*h_l +(1-VV)*rho_l*dhdp_l*der(pp))) - AA*ll*der(pp) +AA*rho_a*h_a*der(la) -
AA*rho_b*h_b*der(lb);
...
//Dh_lv = h_l - h_v;
//Dr_lv = rho_l - rho_v;
// Gamma_a =Units.HeatExchangers.MovingBoundaryOLD.Functions.Gamma(
//       hh=h_a,
//       rho_l=rho_l,
//       rho_v=rho_v,
//       h_l=h_l,
//       h_v=h_v);
// Gamma_b =Units.HeatExchangers.MovingBoundaryOLD.Functions.Gamma(
//       hh=h_b,
//       rho_l=rho_l,
//       rho_v=rho_v,
//       h_l=h_l,
//       h_v=h_v);
// GG = Gamma_a/Gamma_b;
// Theta_a =Units.HeatExchangers.MovingBoundaryOLD.Functions.Theta(
//       hh=h_a,
//       h_l=h_l,
//       drdp_l=drdp_l,
//       rho_l=rho_l,
...
//       rho_v=rho_v,
//       dhdp_v=dhdp_v);
//Theta_b =Units.HeatExchangers.MovingBoundaryOLD.Functions.Theta(
//       hh=h_b,
//       h_l=h_l,
//       drdp_l=drdp_l,
//       rho_l=rho_l,
...
if alone then
  dhdt_a = der(h_a);
  dhdt_b = der(h_b);
elseif not alone and FL ==1 and GR == 0 then
  dhdt_a = dhdp_l*der(pp);
  dhdt_b = der(h_b);
elseif not alone and DR ==1 and GR == 0 then
...
/* Boundaries and connectors */
  ...
  annotation (Diagram(coordinateSystem(preserveAspectRatio=false, extent={{-100,
       ...
       <p><big>The assumptions for this model are:
         ...
         <li>The velocity of the fuid is uniform on the cross sectional area. Homogeneous two-phase flow.
...
<p>
<img src=\"modelica://Components/Resources/Images/RateMass_TP.png\">
</p> 

<p><big> The rate of energy change for the two-phase flow is defined as:
<p>
<img src=\"modelica://Components/Resources/Images/RateEnergy_TP.png\">
</p> 
...
<p>
<img src=\"modelica://Components/Resources/Images/AveVoidFrac.png\">
</p> 
