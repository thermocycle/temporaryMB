...
model Step_6_CrossCond
replaceable package Medium =
      ThermoCycle.Media.SES36_CP;
      replaceable package Medium2 = ExternalMedia.Examples.WaterCoolProp;
parameter Real k_Vol_rec = 1;
parameter Real k_Mass_rec = 1;

  Components.Evaporator.Incompressible.Unit.EvaGwInc evaGw(redeclare package
      Medium =                                                                     Medium,
    Ltotal=66.6,
    AA=0.0007,
    YY=0.243,
    ...
    Tstartsf=398.15,
    DTstartsf=278.15)
    annotation (Placement(transformation(extent={{-42,42},
            {8,82}})));
  ...
  ThermoCycle.Components.Units.ExpansionAndCompressionMachines.Expander expander(
    ...
    p_ex_start=109301,
    T_su_start=398.05)
    annotation (Placement(transformation(extent={{52,6},{76,30}})));
 ParametrizedModels.PdropHP_143 pdropHP_143_1(redeclare package Medium =           Medium)
  ...
  ThermoCycle.Components.Units.HeatExchangers.CrossCondenser crossCondenser(redeclare
  ...
  redeclare package Medium2 = Medium2,
    ...
    Tstart_sf_in=295.15,
    Tstart_sf_out=289.15,
    T_sat_start=301.15)
    annotation (Placement(transformation(extent={{6,-108},{58,-68}})));

  ThermoCycle.Components.FluidFlow.Reservoirs.SourceMdot sourceMdotCrossCondenser(
    redeclare package Medium = Medium2,
    Mdot_0=0.3061,
    p=810927,
    T_0=297.89)
    annotation (Placement(transformation(extent={{-30,-82},{-10,-62}})));
  ThermoCycle.Components.FluidFlow.Reservoirs.SinkP sinkPCrossCondenser(
    redeclare package Medium = Medium2,
    h=2.1E4,
    p0=100000)
    annotation (Placement(transformation(extent={{-38,-122},{-58,-102}})));
equation
  ...
  connect(expander.flange_elc, generator.shaft) annotation (Line(
      points={{72,19},{84,19},{84,28},{97.4,28}},
      color={0,0,0},
      smooth=Smooth.None));
  ...
  connect(pdropHP_143_1.OutFlow, expander.InFlow) annotation (Line(
      points={{47,50},{54,50},{54,22.6},{57.2,22.6}},
      color={0,0,255},
      smooth=Smooth.None));
  ...
  connect(Recuperator.outlet_fl2, crossCondenser.InFlow_fl1) annotation (Line(
      points={{9.28,-47.68},{9.28,-56},{37.2,-56},{37.2,-74}},
      color={0,0,255},
      smooth=Smooth.None));
  connect(crossCondenser.OutFlow_fl1, sinkP.flangeB) annotation (Line(
      points={{37.2,-102},{36,-102},{36,-104},{-18,-104},{-18,-84},{-61.6,-84}},
      color={0,0,255},
      smooth=Smooth.None));

  connect(sourceMdotCrossCondenser.flangeB, crossCondenser.OutFlow_fl2)
    annotation (Line(
      points={{-11,-72},{2,-72},{2,-78},{11.2,-78}},
      color={0,0,255},
      smooth=Smooth.None));
  connect(sinkPCrossCondenser.flangeB, crossCondenser.InFlow_fl2) annotation (
      Line(
      points={{-39.6,-112},{-14,-112},{-14,-114},{11.2,-114},{11.2,-98}},
      color={0,0,255},
      smooth=Smooth.None));
  annotation (Diagram(coordinateSystem(preserveAspectRatio=false, extent={{-120,
            -120},{140,120}}), graphics), Icon(coordinateSystem(extent={{-120,-120},
            {140,120}})));
end Step_6_CrossCond;
