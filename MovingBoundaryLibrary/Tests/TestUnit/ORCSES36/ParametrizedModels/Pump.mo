within MovingBoundaryLibrary.Tests.TestUnit.ORCSES36.ParametrizedModels;
model Pump "Pump model"
  ...
  parameter Boolean imposeMassFlow = true "Set true when using ORCNext option";
public
  ...
  Modelica.Blocks.Interfaces.RealInput M_dot_ext if imposeMassFlow
    annotation (Placement(transformation(extent={{-54,70},{-14,110}}),
        iconTransformation(
        extent={{11,-11},{-11,11}},
        rotation=90,
        origin={7,85})));
         Modelica.Blocks.Interfaces.RealOutput M_dot_aux;
...
/*Fluid properties*/
  ...
  M_dot = if not imposeMassFlow then M_dot_aux else M_dot_aux;
  V_dot = 1;
  elseif (PumpType == PumpTypes.SQThesis) then
    eta_in = 0.931 - 0.108*log10(X_pp) - 0.204*log10(X_pp)^2 - 0.05954*log10(
    X_pp)^3;
    M_dot = if not imposeMassFlow then V_dot*rho_su else M_dot_aux;
...
/* BOUNDARY CONDITIONS */
  ...
  connect(M_dot_aux,M_dot_ext);
  if not imposeMassFlow then
    M_dot_aux = 0;
  end if;
