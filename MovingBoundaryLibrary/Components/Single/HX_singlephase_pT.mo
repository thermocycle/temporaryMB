within MovingBoundaryLibrary.Components.Single;
model HX_singlephase_pT
  ...
  parameter Modelica.SIunits.AbsolutePressure p_cf_start=50E5
    ...
    parameter Modelica.SIunits.AbsolutePressure p_hf_start=50E5
    "Initial pressure of the cold working fluid" annotation (Dialog(tab="Initialization"));
 ...
 parameter Boolean Cp_constant=false
  ...
  Modelica.SIunits.SpecificEnthalpy h_hf_ex(start=Medium2.specificEnthalpy_pT(p_hf_start,T_hf_ex_start));
...
/******************************* CONNECTORS *****************************/
  ...
  ThermoCycle.Interfaces.Fluid.FlangeA inlet_hf(redeclare package Medium = Medium2)
    annotation (Placement(transformation(extent={{80,62},{100,82}}),
        iconTransformation(extent={{80,62},{100,82}})));
  ThermoCycle.Interfaces.Fluid.FlangeB outlet_hf(redeclare package Medium = Medium2)
...
equation
  ...
  stateIn_hf = Medium2.setState_ph(inlet_hf.p,inStream(inlet_hf.h_outflow));
  ...
  LMTD_hf = homotopy(ThermoCycle.Functions.LMTD_robust(stateOut_hf.T - T_w_1, stateIn_hf.T - T_w_2),
    max(0, pinch_hf));

  pinch_cf = min(T_w_1-stateIn_cf.T,T_w_2-stateOut_cf.T);
  pinch_hf = min(stateOut_hf.T-T_w_1,stateIn_hf.T-T_w_2);
  ...
  Q_dot_hf = inlet_hf.m_flow * Cp_hf * (stateIn_hf.T - stateOut_hf.T);

/* Metal wall energy balance */
  M_wall * c_wall * der(T_wall) = Q_dot_hf - Q_dot_cf;

  /* Linear temperature gradient in the wall */
  T_w_2 - T_w_1 = (AU_cf*(stateOut_cf.T-stateIn_cf.T) + AU_hf*(stateIn_hf.T-stateOut_hf.T))/(AU_cf+AU_hf);
  T_wall = T_w_1 + (T_w_2 - T_w_1)/2;

  stateOut_cf = Medium1.setState_ph(outlet_cf.p,h_cf_ex);
  stateOut_hf = Medium2.setState_ph(outlet_hf.p,h_hf_ex);
...
/* BOUNDARY CONDITIONS */
  ...
  inlet_hf.h_outflow=inStream(outlet_hf.h_outflow);
  outlet_hf.h_outflow=stateOut_hf.h;
