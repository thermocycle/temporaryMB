...
model MBeva_Complete_NTU_Switching
...
parameter Modelica.SIunits.Length L_min = 10-4;
...
Q_SH = cp_SH* (M_dot_B +M_dot_ex)/2*epsilon_SH_pf*(TwSH - T_TP)
...
// if (T_htf_ex_SF_measured >= T_htf_ex_SF_min)  and  (T_htf_ex_SF_measured <= T_htf_ex_SF_max) then
...
end MBeva_Complete_NTU_Switching;
