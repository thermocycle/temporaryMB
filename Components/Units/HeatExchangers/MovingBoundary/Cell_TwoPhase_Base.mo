...
model Cell_TwoPhase_Base "1-D lumped fluid flow model for two-phase flow"
...
parameter Modelica.SIunits.Pressure pstart "Fluid pressure start value"
  ...
  parameter Medium.SpecificEnthalpy hstart_a= 763000 "Start value of enthalpy"
    annotation (Dialog(tab="Initialization"));
  parameter Medium.SpecificEnthalpy hstart_b= 2.7E6 "Start value of enthalpy"
    annotation (Dialog(tab="Initialization"));
...
/***************  VARIABLES ******************/
  ...
  Medium.Density rho "Density of the homogeneous fluid";
  ...
  Modelica.SIunits.SpecificEnthalpy h_b(start=hstart_a)
    "Enthalpy state variable at inlet node";
  Modelica.SIunits.SpecificEnthalpy h_a(start=hstart_b)
  ...
  Modelica.SIunits.Volume Volume = AA*ll;
  Modelica.SIunits.Mass MM = rho*Volume;
 Real dVVdt;
 Real VV;
 Real Gamma_a;
 Real Gamma_b;
...
+ rho_a*AA*dzdt_a - rho_b*AA*dzdt_b = M_dot_a - M_dot_b;

/* ENERGY BALANCE */
AA*(der(ll)*(VV*rho_v*h_v + (1-VV)*rho_l*h_l) + ll*(dVVdt*(rho_v*h_v - rho_l*h_l)+VV*drdp_v*der(pp)*h_v
+VV*rho_v*dhdp_v*der(pp) +(1-VV)*drdp_l*der(pp)*h_l +(1-VV)*rho_l*dhdp_l*der(pp))) - AA*ll*der(pp) +AA*rho_a*h_a*dzdt_a -
AA*rho_b*h_b*dzdt_b = M_dot_a*h_a - M_dot_b*h_b +q_dot;
